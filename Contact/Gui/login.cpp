  ...
  ui(new Ui::Login)
{
  ...
  ui->horizontalFrame->installEventFilter(this);
  ui->frame->installEventFilter(this);
  ui->widget->installEventFilter(this);
  isPressedWidget = false;

  // 窗体风格
  this->setWindowFlags(Qt::SplashScreen | Qt::FramelessWindowHint); // WindowStaysOnTopHint窗口顶置
  // 窗体透明
  ...
  connect(this->tcpClient, SIGNAL(error(QAbstractSocket::SocketError)), this,
          SLOT(onSocketErrorChange(QAbstractSocket::SocketError)));
...
{
  if ((obj == ui->horizontalFrame) || (obj == ui->frame)) {
    //    qDebug() << "get topbar！！！！";
...
{
  QDesktopServices::openUrl(QUrl(QLatin1String("C:/Users/Dell/Desktop/Contact/Gui/front/index.html")));
}
...
void Login::on_comboBox_currentIndexChanged(int index)
{
  ...
  // ui->label_4->setStyleSheet(QString("border-image: url(%1);border-width:0px;border-style:solid;border-color:
  // rgb(255, 255, 255);border-radius:33px;").arg(icon1));
}
    ...
    connect(&this->timesignin, &QTimer::timeout, this, [ = ](){
      if (this->timesignin.isActive())
      {
        ...
        if (isuser)
        {
          ...
          {
            qDebug() << "2";
            if (ui->lineEdit_2->text() == "23101116") {
              LoginWarning *myloginwarning = new LoginWarning();
              myloginwarning->show();
              return;
            }
            QByteArray straccount = this->account.toUtf8();
            ...
            // 显示系统托盘图标
            //            w->showicon();
            connect(w, SIGNAL(sendquitData()), this, SLOT(on_pushButton_8_clicked()));
          }
...
void Login::deleteaccount(int i) // 传进来的是标记数字
{
  ...
  QMessageBox *message = new QMessageBox(QMessageBox::Question,
                                         tr("提示"),
                                         tr("是否要删除该账号信息"),
  ...
  switch (result)
  {
    case QMessageBox::Yes:
      ...
      if (infoList.length() == 0)
      {
        ...
        // ui->label_4->setStyleSheet("border-image:
        // url(:/lib/fdogicon.png);border-width:0px;border-style:solid;border-color: rgb(255, 255,
        // 255);border-radius:33px;");
...
void Login::on_toolButton_5_clicked()
{
  ui->label_5->hide();
  ui->toolButton_3->hide();
  ui->pushButton_3->hide();
