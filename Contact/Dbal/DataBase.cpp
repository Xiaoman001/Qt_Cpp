...
bool Usersql::connData()
{
  ...
  {
    this->dbconn = QSqlDatabase::addDatabase("QMYSQL3");
  }
  this->dbconn.setHostName("106.15.55.49");
  this->dbconn.setDatabaseName("contactsoftware");
  if (this->dbconn.open("contactsoftware", "4YWcX87SEp6wRH6e"))
  {
...
void Usersql::queryUserInfo(const QString& account)
{
  ...
  while (query.next())
  {
    if (account == (this->query.value(0).toString()))
    {
      ...
      this->phone      = this->query.value(1).toString();
      this->name       = this->query.value(2).toString();
      this->passwd     = this->query.value(3).toString();
      this->signature  = this->query.value(4).toString();
      this->icon       = getIconUrl(this->query.value(5).toString());
...
void Usersql::AccountIP(const QString& ip)
{
  ...
  QString s = QString("update state set isline=%1,ip=\"%2\" where account=%3").arg(QString::number(this->sate),
                                                                                   ip,
                                                                                   this->getAccount());
  this->query.exec(s);
}
...
void Usersql::setverify(const QString& time,
...
void Usersql::getverify(const QString& time,
                        const QString& otheraccount,
                        const QString& sate,
                        const QString& name,
                        const QString& grouping,
...
{
  QString data = QString("select * from setverify where account='%1' and otheraccount = '%2' ").arg(account,
                                                                                                    otheraccount);
  this->query.exec(data);
  while (query.next())
  {
...
void Usersql::updatagrouping(const QString& otheraccount, const QString& account)
{
  ...
  qDebug() << "由" + otheraccount + "向" + account + "发起的添加好友申请，如果" + account + "同意，"
                                                                              "就将" + account + "的名字设置为" +
    otheraccountname + "放在" + otheraccount + "的" + otheraccountgrouping + "分组下";
  ...
  qDebug() << "账号" + account + "接收到" + otheraccount + "发来的添加好友申请，如果" + account + "同意，"
                                                                                 "就将" + otheraccount + "的名字设置为" +
    accountname + "放在" + account + "的" + accountgrouping + "分组下";
  data = QString("INSERT INTO friend VALUES('123','%1','%2','%3','%4')")
...
QString Usersql::getIpAndPort(QString account) {
  QString data = QString("select * from state where account='%1'").arg(account);
  this->query.exec(data);

  while (query.next())
  {
    return this->query.value(2).toString() + "|" + this->query.value(3).toString();
  }
}

QString Usersql::getPhotoUrlByAccount(QString account)
{
  if (account.size() == 6) {
    QString data = QString("select * from `group` where account='%1'").arg(account);
    this->query.exec(data);
  ...
  QString data = QString("select * from user where account='%1'").arg(account);
  this->query.exec(data);

  while (query.next())
  {
    return this->query.value(5).toString();
  }
}

QString Usersql::getNameByAccount(QString account) {
  QString data;
  if (account.size() == 6) {
    data = QString("select * from `group` where account='%1'").arg(account);
    this->query.exec(data);

    while (query.next())
    {
      return this->query.value(1).toString();
    }
  }
  data = QString("select * from user where account='%1'").arg(account);
  this->query.exec(data);
  while (query.next())
  {
    return this->query.value(2).toString();
...
QStringList Usersql::getRecordByAccount(QString account) {
  QString data = QString("SELECT *FROM message_record WHERE account ='%1' OR receiveaccount = '%2'").arg(account,
                                                                                                         account);
  qDebug() << data;
  this->query.exec(data);
  QStringList ans;
  while (query.next())
  {
    ans.append(this->query.value(0).toString() + "|" + this->query.value(
                 1).toString() + "|" + this->query.value(3).toString());
  }
...
/**
 * @brief Usersql::getUserRole
 * @param account
 * @return
 */
int Usersql::getUserRole(const QString& account) {
  QString data = QString("select role from group_membership where user_account='%1'").arg(account);
  this->query.exec(data);
  query.next();
...
/**
 * @brief Usersql::getUserGroup
 * @param account
 * @return
 */
QStringList Usersql::getUserGroup(const QString& account) {
  QString data = QString("select user_account from group_membership where group_account='546432'");
  this->query.exec(data);
  QStringList resultlist;
  while (query.next()) {
    qDebug() << "resultlist" << query.value(0).toString();
