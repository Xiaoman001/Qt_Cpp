...
public:
  ...
  Usersql();
  bool    connData();                                             // 连接数据库
  bool    queryUser(const QString& user,
                    const QString& password);                     // 查询是否有该账户
  ...
  void    setverify(const QString& time,
                    const QString& account,
                    const QString& sate,
                    const QString& name,
                    const QString& grouping,
                    const QString& otheraccount); // 添加好友数据
  void getverify(const QString& time,
                 const QString& otheraccount,
                 const QString& sate,
                 const QString& name,
                 const QString& grouping,
                 const QString& account);                     // 添加好友数据
  QString     getverifygrouping(const QString& account,
                                const QString& otheraccount); // 获取主动添加方设置的分组
  void        updatagrouping(const QString& otheraccount,
                             const QString& account);         // 暂无
  ...
  QString     getIpAndPort(QString account);
  QString     getPhotoUrlByAccount(QString account);
  QString     getNameByAccount(QString account);
  QStringList getRecordByAccount(QString account);
  int         getUserRole(const QString& account);  // 根据账号获取权限
  QStringList getUserGroup(const QString& account); // 根据群号获取账号
};
